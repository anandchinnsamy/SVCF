...
public class Approvalfortransfer {
	WebDriver d;
...
@Test(priority =1)
public void trnsferapproval() throws InterruptedException
{
	...
	try {
		...
		WebElement hover = d.findElement(By.xpath("//*[@id=\"smoothmenu_h\"]/ul/li[3]/ul/li[4]/ul/li[2]/a"));
		action.moveToElement(hover).build().perform();
		hover.click();
		logger.info("Mouse is successfully clicked on transfer approval screen");
	    }
	catch (Exception e)
	{
		logger.error(e);
		logger.error("Mouse is successfully  not clicked on transfer approval screen");
	}
	Thread.sleep(2000);
...
public void suggestforapproval() throws InterruptedException
{
	...
	// clicking yes button
	try {
		 WebElement pw =d.findElement(By.name("ctl00$cphMainContent$btnInfo_yes"));
			pw.click();
	...
	if(status.equals(sts.getText()))
	{
		...
		logger.info("Transfer is approved successfully");
	}
	else
	{
		
		logger.error("Transfer is not approved");
	}
