...
public class CashReceiptOtherBranch {
	 WebDriver d;
...
//CRR
 ...
 try
 {
 	...
 	Thread.sleep(1000);
		...
		WebElement branch =d.findElement(By.xpath("//*[@id=\"smoothmenu_h\"]/ul/li[2]/ul/li[1]/ul/li[1]/ul/li[2]/a"));
		action.moveToElement(branch).build().perform();
		branch.click();
	...
	//	designation.selectByIndex(2);
		logger.info("Dindugal is successfully selected");
		
	}
	
	catch(Exception e)
	{
		logger.error("Dindugal is not successfully selected");
	}
	...
	branch.selectByVisibleText("DLK-4/6");
	logger.info("Chit is selected successfully");
	}
	catch(Exception e)
	{
		logger.error("Chit is not selected");
	  ...
	  try
	    {
			...
			branch.click();
			logger.info("Cash receipt for other branch is added successfully" );
	    }
