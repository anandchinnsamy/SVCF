...
public class Chitgroupviewedit {
	WebDriver d;
	...
	public void groupadditionhover() throws InterruptedException
	{
		...
		try {
			Actions action =new Actions(d);
			 ...
			 WebElement pw =d.findElement(By.xpath("//*[@id=\"smoothmenu_h\"]/ul/li[3]/ul/li[1]/ul/li[2]/a"));
			 action.moveToElement(pw).click().build().perform();	
			 
			logger.info("Hovered on Chit Group View /Edit screen successfully");
		} 
		catch (Exception e) {

			logger.error("Not hovered on chit group view/edit screen");

		}
	
	}
	@Test(priority =2)
	public void chitview() throws InterruptedException {
		
		
		try {
			Actions action =new Actions(d);
			
			 WebElement pw =d.findElement(By.name("ctl00$cphMainContent$gridBranch$DXFREditorcol2"));
			  pw.sendKeys("chitgroup");
			 logger.info("Group is searched successfully");
			 
		} 
		
		catch (Exception e) {

			logger.error("Group is not searched");
		...
		try {
			Actions action =new Actions(d);
			 WebElement pw =d.findElement(By.xpath("//*[@id=\"cphMainContent_gridBranch_DXDataRow0\"]/td[1]/img[2]"));
			  pw.click();
			  Thread.sleep(1000);
			...
			logger.error("Group is not deleted");
